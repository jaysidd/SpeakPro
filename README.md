...
- **Natural sentence pacing** — paragraphs are split into sentences (with abbreviation guards for `Dr.`, `Mr.`, `e.g.`, etc. and digit-version protection for `v1.0`), each spoken as its own queue item. Short pause (~0.5 s) at sentence boundaries, longer pause (~1 s) between paragraphs — no more breathless wall-of-speech.
- **Sentence-level queue** — long answers are spoken one sentence at a time, so you can `speakpro skip` past whatever you don't care about.
